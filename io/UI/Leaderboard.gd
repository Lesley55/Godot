...
#		if sorted[i] == current_score: # doesn't work because type changes
		if sorted[i].playerName == _current_score.playerName and sorted[i].color == _current_score.color and sorted[i].score == _current_score.score:
